...
app.get('/bfs', async ({ query }) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        worker.on('error', (err: Error) => {
            reject({ status: 500, body: { error: 'Worker error', details: err.message } });
        });
...
// Start the server
app.listen(PORT, () => {
    console.log(`Server is running on http://localhost:${PORT}/bfs?startNode=${nodeNo(PORT)}`);
});
